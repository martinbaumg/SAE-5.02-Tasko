...
class DatabaseConnection:
    ...
    def connect(self): # We connect to the database
        try:
            ...
            if self.connection.is_connected(): # If the connection is established, we print a message
                print('Connected to MySQL database')
                self.cursor = self.connection.cursor()  # Create a cursor
                return self.connection
        except Error as e: # If the connection is not established, we print an error message
    ...
    def execute_query(self, query, data=None): # We define a method to execute a query
        try:
            if data:
                self.cursor.execute(query, data)  # Execute the query with parameter values
            else:
...
class main: # We create a class main
    def __init__(self):
        self.db = DatabaseConnection() # We create an instance of the class DatabaseConnection
